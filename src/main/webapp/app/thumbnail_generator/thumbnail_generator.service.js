(function() {
	...
	.service('ThumbnailGenerator', ThumbnailGenerator);

	ThumbnailGenerator.$inject = ['$q'];

	function ThumbnailGenerator ($q) {	
		...
		var scaling,thumbnailType,encodingOption;

		function generateThumbnail(img,opts){

			var t0 = performance.now();

			scaling 		=  opts.scaling 		|| 	defaultOption.scaling;
			thumbnailType   =  opts.thumbnailType 	|| 	defaultOption.thumbnailType;
			encodingOption  =  opts.encodingOption 	|| 	defaultOption.encodingOption;
			...
			var scale 	= normaliseScale(scaling);
			var sw 	= img.width;  // Source Image Width           
			var sh 	= img.height; // Source Image Height
			...
			//Return the base64 string of Thumbnail image to caller
			return cv.toDataURL(thumbnailType,encodingOption);


			//context.fillText('canvas scale, less blurry / more noisy', 50, 220);

			var t1 = performance.now();
			console.log("total time taken : " + (t1 - t0) + " milliseconds.");
			alert("total time taken : " + (t1 - t0) + " milliseconds.");
